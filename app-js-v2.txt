...
function createNewItem(area = "Casa") {
  ...
  render();
  showToast("Nuova cosa aperta creata.");
}
...
function renderMobileSections() {
  ...
  if (state.view === "areas") {
    const sections = areaForColumns()
      .map((area) => mobileSection(area, items.filter((item) => item.area === area)))
      .join("");
...
function renderMobileCard(item) {
  ...
  return `
    <button class="mobile-card" data-open-id="${escapeAttr(item.id)}">
      ...
      <span>
        <span class="mobile-card-title">${escapeHtml(item.title)}</span>
      ...
      <article class="detail-card">
        <div class="detail-top">
          <div style="min-width:0; flex:1">
            <input class="detail-title-input" id="titleInput" value="${escapeAttr(item.title)}" aria-label="Titolo" />
          </div>
        ...
        <div class="form-grid">
          ...
          <div class="form-field full">
            ...
            <textarea class="textarea" id="stepInput" maxlength="220">${escapeHtml(item.nextStep || "")}</textarea>
          </div>

          <div class="form-field full">
            <label for="blockerInput">Bloccante</label>
            <textarea class="textarea" id="blockerInput" maxlength="220">${escapeHtml(item.blocker || "")}</textarea>
          </div>
          ...
          <div class="form-field full">
            ...
            <textarea class="textarea" id="notesInput" maxlength="800">${escapeHtml(item.notes || "")}</textarea>
          </div>
        </div>
...
function bindEvents() {
  ...
  if (backBtn) backBtn.addEventListener("click", () => { state.selectedId = null; render(); });

  const saveBtn = document.getElementById("saveBtn");
  if (saveBtn) saveBtn.addEventListener("click", saveCurrentDetail);

  const closeBtn = document.getElementById("closeBtn");
  if (closeBtn) closeBtn.addEventListener("click", () => {
    saveCurrentDetail({ status: STATUS.CLOSED });
    state.selectedId = null;
    render();
  ...
  if (archiveBtn) archiveBtn.addEventListener("click", () => {
    saveCurrentDetail({ status: STATUS.ARCHIVED });
    state.selectedId = null;
    render();
...
function bindDefaultTextAutoClear() {
  ["titleInput", "stepInput", "blockerInput", "notesInput"].forEach((id) => {
    ...
    el.addEventListener("focus", () => {
      ...
      if (!DEFAULT_FIELD_VALUES.has(currentValue)) return;
...
function saveCurrentDetail(extraPatch = {}, leaveDetail = true) {
  ...
  const patch = {
    title: document.getElementById("titleInput")?.value.trim() || "Senza titolo",
    area: document.getElementById("areaSelect")?.value || "Casa",
    status: document.getElementById("statusSelect")?.value || STATUS.OPEN,
    nextStep: document.getElementById("stepInput")?.value.trim() || "",
    blocker: document.getElementById("blockerInput")?.value.trim() || "",
