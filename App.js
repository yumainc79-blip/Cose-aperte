...
function getActiveAreaCount(area) {
  return state.items.filter((item) => isActiveItem(item) && item.area === area).length;
}

function getNonEmptyAreas() {
  return AREAS.filter((area) => getActiveAreaCount(area) > 0);
}
...
function renderDashboard() {
  ...
  const columns = state.view === "areas" ? getNonEmptyAreas() : areaForColumns();
  ...
  return renderAppChrome(`
    ...
    <section class="content-grid">
      <div class="board" aria-label="Panoramica per aree">
        ${columns.length ? columns.map((area) => renderColumn(area)).join("") : `<div class="empty-state"><strong>Nessuna area attiva.</strong>Aggiungi una nuova cosa aperta per creare la prima area.</div>`}
      </div>
  ...
  return `
    <section class="column">
      <div class="column-header">
        ...
        <small>${getActiveAreaCount(area)}</small>
      </div>
      <div class="card-list">
...
function renderMobileAreaSections(items) {
  const areas = getNonEmptyAreas();
  if (!areas.length) {
    return `<div class="empty-state"><strong>Nessuna area attiva.</strong>Aggiungi una nuova cosa aperta per creare la prima area.</div>`;
  }

  return areas.map((area) => {
    const areaItems = sortItems(items.filter((item) => item.area === area));
    const count = getActiveAreaCount(area);
    ...
    return `
      <section class="section area-section">
        ...
        ${preview || `<div class="empty-state compact"><strong>Nessun risultato.</strong>Prova a modificare la ricerca.</div>`}
      </section>
    `;
  }).join("");
}
